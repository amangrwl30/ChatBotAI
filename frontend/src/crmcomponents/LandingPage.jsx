...
const LandingPage = ({ onWebsiteSubmit }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (website.trim() && isValidWebsite(website)) {
      setIsLoading(true);
      // Call the parent's onWebsiteSubmit
  ...
  return (
    <div className={`${sharedState.yellowAiTheme ? 'dark' : ''}`}>
      <div 
        className="landing-container flex flex-col items-center justify-center h-screen"
        ...
        {isLoading ? (
          ...
          <div className="text-center p-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col md:flex-row items-center bg-white mt-20 p-2 px-16 rounded-full shadow-xl w-[900px]">
              <span className="mr-2 text-black text-lg w-[23rem]">
                Create an <span className='text-green-500 font-bold'>online assistant </span> for my web site:
              </span> 
              {WebsiteAutoSuggest ? 
            ...
            <button 
              ...
              className="bg-[linear-gradient(to_right,rgb(255,204,51),rgb(245,158,11))] hover:bg-[linear-gradient(to_right,rgb(245,158,11),rgb(255,204,51))] mt-10 w-full text-xl max-w-xs bg-yellow-500 px-8 py-4 rounded-full text-black font-bold shadow-md hover:bg-yellow-600 transition duration-300 ease-in-out transform hover:scale-110"
            >
              Generate
            </button>
          </div>
